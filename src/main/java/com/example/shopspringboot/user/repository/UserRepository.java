 ...
 * 想像它是專門管理資料庫中 UserEntity（會員）這張資料表的「倉庫管理員」。
 * 這裡繼承了 JpaRepository，代表它自動內建了常見的 CRUD 功能（新增、查詢、修改、刪除），
 * 我們不需要自己辛苦寫一堆 SQL 指令（像是 SELECT * FROM user ...）。
 * JpaRepository<UserEntity, Long> 的意思是：
 * - UserEntity: 這個倉庫主要存放的「食材/資料格式」是 UserEntity 實體。
 * - Long: UserEntity 資料表裡面的主鍵（ID）資料型態是長整數（Long）。
 */
public interface UserRepository extends JpaRepository<UserEntity, Long> {
     ...
     * 為什麼用 Optional<UserEntity>？
     * Optional 就像是一個「保險箱」，裡面可能「有 UserEntity」或者「是空的（Null）」。
